...
<!-- todo-guard 운영 규칙 v3 · setup.sh 가 관리한다. 원본은 ~/.claude/skills/todo-guard/rules/claude-md-block.md -->

- 지시는 훅이 받는 순간 TODO.md 에 `#번호` 로 적는다. 직접 등록하지 않고, TODO.md 를 읽거나 고치지 않는다
- 끝나면 마지막 확인 명령 뒤에 붙인다: `… && bash "$HOME/.claude/skills/todo-guard/scripts/todo.sh" done 14`
- 끝나지 않은 것은 done 하지 않는다. 사용자 판단이 필요하면 `todo.sh hold 14 "사유"`, 지시에 없던 일은 `todo.sh add "할 일"`
- 질문(읽고 답만 하는 것)은 그냥 답한다. 파일을 바꾸지 않은 턴의 등록은 훅이 지운다
- TODO.md 맨 위 「항상 지킬 것」은 지킨다. 「앞으로 ~하지 마 · 항상 ~해」는 `bash "$HOME/.claude/skills/todo-guard/scripts/rule.sh" add "규칙"` 으로 등록한다
