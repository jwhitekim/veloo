...
export default function Calendar() {
  const t = useT(); const isMobile = useIsMobile(); const navigate = useNavigate(); const { username = '' } = useParams()
  ...
  return <div className="cal-root"><div className="app-page-intro-shell app-page-intro-shell--workspace">
    <PageHeader kicker="Weekly planner" icon={<CalendarDays />} title={t('calendar.heroTitle')} description={t('calendar.heroDescription')} badge={<><CalendarDays size={14} /> {t('calendar.eventsBadge', { count: events.length })}</>} />
    </div><div className="cal-shell">
    <DndContext sensors={sensors}><section className="cal-workspace">
      <div className="cal-toolbar"><div className="cal-week-nav"><button className="cal-nav-btn app-action app-action--icon" onClick={() => moveWeek(-1)} aria-label={t('calendar.prevWeek')}><ChevronLeft size={16} /></button><span className="cal-week-label">{fmtWeek(weekStart)}</span><button className="cal-nav-btn app-action app-action--icon" onClick={() => moveWeek(1)} aria-label={t('calendar.nextWeek')}><ChevronRight size={16} /></button></div><div className="cal-toolbar-actions"><label className="cal-duration-control"><Clock3 size={15} /><span>{t('calendar.durationLabel')}</span><select value={durationHours} onChange={event => setDurationHours(Number(event.target.value))} aria-label={t('calendar.durationLabel')}>{[1, 2, 3, 4].map(hours => <option key={hours} value={hours}>{t('calendar.durationHours', { count: hours })}</option>)}</select></label><button className="cal-list-btn app-action" onClick={() => navigate(`/${username}/tasks`)}><ListTodo size={15} />{t('calendar.todoListButton')}</button><button className="cal-list-btn app-action" onClick={() => navigate(`/${username}/calendar/review`)}><BarChart3 size={15} />{t('shell.weeklyReview')}</button></div></div>
