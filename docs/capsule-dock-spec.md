...
- **탭(드래그 없이 누르기만, `is-pressed`)**: 색을 바꾸지 않고 `filter: brightness(1.45)`로
  지금 색 그대로 밝기만 올림. 해당 filter는 인디케이터 하나가 아니라 **바(`.shell-mobile-tabs`)
...
- **드래그 중(`is-dragging`)**: 밝기·부풀기 전부 꺼지고 `#c4c4c4` 그대로 유지됨 —
...
인식해 그때까지 올라간 값(예: 1.2)에서 다시 1.0으로 0.15초 동안 되돌아감 — 값이 끝까지
안 올라가고 중간에 방향을 틀어 내려가는 것. 대부분의 드래그는 누른 직후 바로 시작되므로
이 되돌림이 거의 매번 발생해 "밝아지는 게 끝나는 걸 본 적이 없다"는 인상을 줌. 설계
의도("드래그 중엔 밝아지면 안 된다")는 지켜지고 있으나 전환 방식이 버그처럼 보이는 상태 —
`pointerdown` 즉시 `isPressed`를 켜는 대신 지연시키는 시도(60ms)를 해봤으나 드래그 확정
타이밍에 의존하는 방식이라 효과가 없어 되돌림. 근본 수정은 아직 미착수.

부가 요인 후보: 드래그 중 같은 요소(`.shell-mobile-tabs`)에 `transformOrigin`/
`--shell-edge-scale`을 매 프레임 직접 건드리고 있어, 저사양 기기에서는 이 되돌림 전환
자체가 매끄럽지 않고 더 끊겨 보일 가능성 있음(미검증).
