...
탭이 눌린 상태(`is-pressed`)인 동안, 드래그로 이어지는지 여부와 무관하게 캡슐 바 전체(배경·인디케이터·아이콘)가 `brightness(1.2)`로 밝아지고 `scale(1.05)`로 살짝 부풀어야 한다(SHALL). 밝기 전환은 눌렀을 때 0.15초로 올라가고, 손을 뗄 때만 0.15초로 꺼져야 한다(SHALL) — 드래그 시작이 밝기를 끄는 트리거가 되어서는 안 된다(SHALL NOT).
